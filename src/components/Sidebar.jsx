...
export default function Sidebar({ mobileMenuOpen, setMobileMenuOpenfn }) {
  ...
  return (
    <>
      ...
      <div className={`fixed z-[101] top-0 h-full w-2/3 bg-gradient-to-tl from-white/10 to-[#002037]  backdrop-blur-lg  p-6 transition-all duration-500 md:hidden ${mobileMenuOpen ? 'left-0' : '-left-full'}`}>
        <Link onClick={() => setMobileMenuOpenfn(false)} to='/'>
          <img alt='background-image' className='sm:h-9 h-[40px]' src={logo}></img>
        </Link>
